...
* **Button-only sessions:** Session IDs and one-time tokens exist only in hidden
  ...
  `/wekan/path`.
* **Reload behavior:** Reloading a public repository page sends no reusable session
  state and shows the same clean URL as a logged-out user. Authenticated-only pages
  return to `/`. Use Omi's buttons to move between pages while staying logged in.
